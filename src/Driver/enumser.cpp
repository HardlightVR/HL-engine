...
_Return_type_success_(return != 0) BOOL CEnumerateSerial::UsingRegistry(_Out_ CNamesArray& ports)
{
  ...
  nStatus = RegQueryInfoKey(serialCommKey, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &dwMaxValueNameLen, NULL, NULL, NULL);
  if (nStatus != ERROR_SUCCESS)
  {
    SetLastError(nStatus);
    return FALSE;
  ...
  while (bContinueEnumeration)
  {
    ...
    bContinueEnumeration = (RegEnumValue(serialCommKey, dwIndex, valueName.m_pData, &dwValueNameSize, NULL, NULL, NULL, NULL) == ERROR_SUCCESS);
