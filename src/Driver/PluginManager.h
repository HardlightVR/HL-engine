...
private:
	...
	std::pair<DeviceId<local>, std::string> instantiateVirtualDevice(const std::string& plugin, const Parsing::VirtualDeviceDescriptor& device);

	std::vector<std::pair<DeviceId<local>, std::string>> m_virtualDevices;
};
