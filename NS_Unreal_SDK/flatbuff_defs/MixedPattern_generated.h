...
struct PatternT : public flatbuffers::NativeTable {
  std::vector<std::unique_ptr<NullSpace::HapticFiles::Mixed::HapticFrameT>> items;
};
...
inline PatternT *Pattern::UnPack(const flatbuffers::resolver_function_t *resolver) const {
  ...
  { auto _e = items(); if (_e) { for (flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { _o->items.push_back(std::unique_ptr<NullSpace::HapticFiles::Mixed::HapticFrameT>(_e->Get(_i)->UnPack(resolver))); } } };
  return _o;
}
